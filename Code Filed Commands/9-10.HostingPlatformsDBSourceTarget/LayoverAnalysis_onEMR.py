...
spark = SparkSession.builder.appName('LayoverAnalysis_onEMR_usingS3').getOrCreate()

df1 = spark.read.format('csv').option('header', 'true').option('inferSchema', 'true')\
    .load('s3://3aayaam-data-engg-vol-2/3aayaam_Spark/flight_data/flight_data_2.csv')
...
df11.write.mode('overwrite').format('parquet').save('s3://3aayaam-data-engg-vol-2/3aayaam_Spark/output/EMR_step-2_layover_analysis.parquet')
